...
原理是这样的，绘制贝塞尔曲线之前，会先给定几个控制点，再从 0 到 1 之间选择一个参数t，将控制点依次连接成线段，在每个线段上找到 t 分位点，将这些新的 t 分位点连接成新的线段，重复这个过程，到了最后一轮只能得到一个新点，最后一轮找到的点就是贝塞尔曲线上的一个点，我们把它叫做参数 t 对应的点，然后我们枚举0到1之间所有的t值，最终，每个 t 对应的点合起来就构成了贝塞尔曲线，第一个和最后一个控制点分别是曲线的起点和终点。

例如，考虑初始只有三个控制点的情况，这种情况也叫二次贝塞尔曲线（Quadratic Bezier），有三个点B0、B1、B2，B0和B1构成一个线段，B1和B2构成另一个线段，我们将每个线段看作一个0到1的范围，取t值对应的点，得到两个新点，然后再将两个新点连成一条线段，再取t值对应的点，这个点就是t值对应的贝塞尔曲线上的点。如此，我们通过枚举 0~1 之间所有的t，就能找到贝塞尔曲线上的所有点。

这个过程是一个递归插值的过程。

**贝塞尔曲线代数方程（Bezier Curve Algebraic Formula）**

给定 n+1 个控制点，每个 t 需要找 n 轮，可以写出一个 n 阶贝塞尔表达式，如下：

$$
b^{n}(t) = b_{0}^{n}(t) = \sum_{j=0}^{n} b_{j}\, B_{j}^{n}(t)
$$

$b^n(t)$ 表示参数 t 处的点，$b_i^r(t)$ 表示第 r 层递归得到的第 i 个点，$b_0^n(t)$ 表示第 n 层递归得到的第 0 个点，和 $b^n(t)$ 是等价的。
...
它的原理是这样的，先给定 16 个控制点，它们在二维平面上按 4x4 网格分布，它们各自的高度可以不同，整体的绘制过程是一个二重循环，分别遍历不同方向，比如，外层循环遍历 u 方向，内层循环遍历 v 方向，内层循环迭代完成后会得到一条贝塞尔曲线，外层循环迭代完成后就能得到整个贝塞尔曲面。
