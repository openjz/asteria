...
（从眼睛发射一条光线，穿过成像平面上的一个像素，和场景中的物体交于一点，这个像素的着色由交点的着色决定，这个交点的着色又由光源直接照射的光、其他物体反射过来的光、折射过来的光共同决定，就好像有三个光源在同时照射这个点一样。）
...
空间划分的思路是，将包围盒按照树状结构划分，光线和格子求交的时候，按照树的结构计算，及时剪枝，能减少很多计算。
