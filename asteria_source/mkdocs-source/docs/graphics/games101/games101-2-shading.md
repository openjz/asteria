...
令 $n$ 为原始法线向量，$n'$ 为变换后的法线向量，$t$ 为表面上的一个切线向量，$t'$ 为变换后的切线向量，那么 $n^Tt = 0$，我们希望变换后的法线和切线仍然满足垂直关系，即 $n'^Tt' = 0$，又因为 $t' = VMt$，所以 $n'^TVMt = 0$，因此，$n'^TVM = n^T$，$n' = ((VM)^{-1})^T n$。

为什么切线的变换，可以直接用MV矩阵变换呢？可以做个简单的推导，$t' = P_2'-P_1' = VM(P_2-P_1) = VMt$。

**简单来说，M变换（Model变换）有可能会破坏法线和平面的正交关系（因为会做向量缩放操作），而不会破坏切线和平面的平行关系，并且，只有M变换会破坏正交关系，V变换不会（V变换是纯粹的刚体变换，即旋转+平移）**。
...
如果一个正方体在渲染的时候采用上述方法，会渲染出来一个圆角的正方体，如果我们想渲染一个有棱有角的正方体，就不能采用上述方法，而是要把每个三角形面视为一个平坦的平面，计算出每个三角形面的法线，这样渲染出来的正方体就会有棱有角。

为了区分这两种渲染效果，计算机图形学提出了硬边和平滑边的概念：

硬边（Hard Edge）：每个三角形面只有一个法线，渲染出来的效果是有棱有角的，或者，每个顶点有多个法线，对应不同的面，渲染时使用对应面的法线。

平滑边（Smooth Edge）：每个顶点的法线由多个面上的法线取平均得到。
...
1. 光栅化，对每个三角形划定一个边界框，对边界框内的每个像素点进行覆盖测试和深度测试，确定哪些像素点需要渲染。
2. 对于需要渲染的像素点，需要计算出它对应的纹理坐标，由于纹理映射只保存了顶点和纹理坐标之间的映射关系，所以需要通过重心坐标插值来计算出像素点的纹理坐标（$u,v$），$u,v$ 坐标的值通常在0~1之间。
3. 计算纹理坐标的颜色，纹理坐标的颜色可以通过插值来计算，比如双线性插值，就是利用纹理坐标周围的四个纹理像素点的颜色来计算出纹理坐标的颜色。
4. 着色时，将纹理图片上的颜色作为漫反射系数 $k_d$。
...
texture太大，要把它映射到一个小的区域上，这个过程实际上是对纹理降低采样率采样，和三角形着色类似，同样会出现走样问题（摩尔纹，锯齿等）。

从现象上看，出现走样是因为采样率降低，屏幕上的一个像素（即一个采样点）对应了一大块纹理区域。
...
可以把环境贴图想成一个包围场景的球或天空盒，从球面向内部的场景发射环境光，把向场景内部射来的所有方向的环境光照信息记录到纹理中，就是环境贴图。

然后在渲染时根据物体表面的法线方向，从纹理中采样出对应的环境光照信息，再根据物体自身表面的材质，计算物体表面的反射光照。
...
然后对两个切线做叉积可以求出法线方向向量并单位化：

$$
\vec{n} = (-dp/du, -dp/dv, 1).normalize()
$$

注意这里的法线是切线空间的法线（默认为 $(0,0,1)$），还需要利用 TBN 矩阵变换回世界空间。
