...
为什么向量的第三维是0呢？是因为向量是无法平移的，它具有平移不变性，**齐次坐标的引入是为了解决点的平移问题，而不是让向量平移**，如果是对一个向量做变换，为了使向量不受平移变换的影响，它的第三维必须是0。
...
- 平行分量不变
- 垂直分量在垂直于 $\vec{n}$ 的平面里做一个二维旋转

**定义**

将一个向量 $\vec{v}$ 绕向量 $\vec{n}=(n_x,n_y,n_z)$ 旋转 $\alpha$ 角度的旋转矩阵如下所示（$\vec{v}$ 和 $\vec{n}$ 都是单位向量，$\vec{n}$ 是一个过原点的旋转轴）：
...
$$
\begin{aligned}
\vec{v}'_{ro}&= \vec{R}(\vec{n},\alpha)\,\vec{v} \\
&= \cos(\alpha)\,\vec{v}
  + \bigl(1-\cos(\alpha)\bigr)\,(\vec{n}\!\cdot\!\vec{v})\,\vec{n}
  + \sin(\alpha)\,(\vec{n}\!\times\!\vec{v})
\end{aligned}
$$
...
其中，$x,y,z$ 是虚部，$w$ 是实部。也可以简记为：

$$
\vec{q} = (u, w)
$$

其中 $u=(x,y,z)$ 是向量部分，$w$ 是标量部分。
...
$$
p=(a,a_0), \quad q=(b,b_0)
$$

则，

$$
pq = \bigl(a_0 b + b_0 a + a\times b,\ a_0 b_0 - a\cdot b\bigr)
$$

四元数乘法把3维向量中的点积和叉积都编码进去了，这也是四元数特别适合描述3维旋转的原因。
...
反过来顺序会变号，$$ji = -k, \quad kj = -i, \quad ik = -j$$
...
和罗德里格斯公式一样，3维旋转可以用“绕单位轴 $\vec{n}$ 旋转角度 $\theta$”来描述，而使用四元数可以将这组轴角信息描述为一个单位四元数：

$$
q = \left(\sin(\theta/2)\vec{n}, \cos(\theta/2)\right)
$$

再把普通3维向量 $\vec{v}$ 表示成纯四元数：
...
= q\,\vec{v}\,q^{\star} \\
&= \cos(\theta)\,\vec{v}
  + \bigl(1-\cos(\theta)\bigr)\,(\vec{n}\!\cdot\!\vec{v})\,\vec{n}
  + \sin(\theta)\,(\vec{n}\!\times\!\vec{v})
\end{aligned}
$$
...
对于泰特布莱恩角（Tait–Bryan angles），例如，以x-y-z 外旋为例，以下两种转法对物体造成的效果是一样的：
- 先绕 x 轴旋转 20 度，再绕 y 轴旋转 90 度，再绕 z 轴旋转 20 度。
- 先绕 x 轴旋转 40 度，再绕 y 轴旋转 90度，不再做第三次旋转。

也就是说，只要绕 y 轴旋转 90度，绕 z 轴旋转和最开始绕 x 轴旋转造成的效果是一样的，那其实没必要绕 z 轴旋转了，直接在最开始绕 x 轴一步旋转到位即可，最后绕 z 轴的旋转实际上是冗余操作。

这就是欧拉角万向节死锁（Gimbal Lock）现象。

再比如，对于经典欧拉角（Proper Euler Angle），以z-x-z 外旋为例，以下两种转法是一样的：

- 绕 z 轴转 20 度，再绕x轴旋转0度或180度，再绕 z 轴转 20 度。
- 绕 z 轴转 40 度，再绕x轴旋转0度或180度，不再做第三次旋转。

**网上很多文章拿万向陀螺仪来解释这个问题，是不对的**，万向陀螺仪的死锁和欧拉角的死锁是两个完全不同的概念，万向陀螺仪的死锁是机械结构限制导致的旋转自由度受限问题，而欧拉角的死锁是在特定旋转顺序和角度组合下的旋转冗余问题。
...
>上面提到的任意立方体，概念上对应的是透视投影的“视锥”，正式的名称叫做观察体（Viewing Volume），经过投影变换后，观察体变换为裁剪体（Clipping Volume），裁剪体是一个标准立方体，投影变换将坐标变换为 NDC 坐标（Normalized Device Coordinates，规范化设备坐标）。观察体的大小和位置是人为定义的，取决于我们想要看到什么样的场景。

>上面的正交投影变换是将缩放矩阵和平移矩阵组合起来，即使观察体的中心已经对准原点，平移矩阵也不能省略，因为之前视图变换是把相机放在了原点，而观察体通常在相机的前面一段距离，并不在原点上，平移操作将观察体的中心放到原点，便于后续的处理。

### 透视投影 (Perspective projection)

透视投影中，相机在原点，相机和视角围成一个四棱锥体（Frustum），相机前面有两个平面，近景平面和远景平面，近景平面和远景平面分别截出这个四棱锥体的顶部和底部（截取后叫平截头体）。
...
挤压过程中，近景平面上的所有点不变，远景平面上点的 z 坐标不变，尺寸变为和近景平面一样的大小，远景平面的中心点不变，**而其他位于远景平面和近景平面中间的点，他们的 z 坐标是会发生变化的**，也就是，挤压不仅是两侧挤压，还会对点的 z 坐标重新排布。

四棱锥远景平面和近景平面的 y 坐标呈现这样的关系（$y' = \frac{n}{z}y$, $x' = \frac{n}{z}x$）：

![persp2ortho.png](/img/games101-notes/persp2ortho.png)

**推导过程**：

利用了齐次坐标同时乘以一个数坐标不变的性质，即 $[x,y,z,1]^T$ 和 $[kx,ky,kz,k]^T$ 表示同一个点。

那么，点 $[x,y,z,1]^T$ 应该会经过这样一个变换过程，z 坐标被变换成什么暂时不知道，
...
$$
M_{\mathrm{persp}\to\mathrm{ortho}}^{(4\times 4)}
\begin{pmatrix}
x\\
y\\
z\\
...
利用近景平面上点不变的条件，我们将近景平面上的点代入变换，得到：
...
可知，第三行必然是 $[0,0,a,b]$ 这样的结构，再利用远景平面上点的 z 坐标不变的条件，得到：
...
判断像素是否在三角形内部这个过程也叫**覆盖测试**（Coverage Test）。
...
摩尔纹（Moire Patterns），比如跳过奇数行和奇数列就能形成
...
根据奈奎斯特采样定理，采样频率必须至少是信号频率的两倍，才能避免走样现象。
...
在深度测试时，不能直接用透视投影后的z坐标做深度值，因为对于透视投影来说，挤压视锥体的过程会改变z坐标的线性关系，如果直接用透视投影变换后的z坐标做深度值，会导致深度测试错误，因此，我们需要使用投影变换之前，也就是在视图空间下的z坐标做深度值，这样才能保证深度测试的正确性。

这里利用到一个性质，经过投影变换后，点的齐次坐标 $w$ 的值刚好等于变换前的 z 坐标的值（即在视图空间下的z值）。

然后，通过对齐次坐标做插值，就可以得到任意点的深度值，一般采用以下公式：
...
其中，$0,1,2$ 分别代表三角形的三个顶点，$w_0,w_1,w_2$ 是三角形顶点的齐次坐标，$\alpha,\beta,\gamma$ 是重心坐标，$z_0$、$z_1$、$z_2$ 是三角形顶点在视图空间下的深度值，因此，$z_p$ 实际上就等于 $Z$。
