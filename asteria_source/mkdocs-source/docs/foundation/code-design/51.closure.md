...
可以把函数对象理解成“一种能保存状态、也能像函数一样调用的对象”。

从效果上看，它和闭包很像，因为它也能把数据和行为放在一起。

不过两者还是有一点区别：

1. 闭包强调的是“函数捕获了定义时的外部环境”。
2. 函数对象强调的是“对象自己带着状态，并且实现了调用操作”。

在 C++ 里，这两者又是非常接近的。因为 lambda 表达式在底层也会生成一个匿名的函数对象，捕获到的变量会变成这个对象里的成员。

所以可以这样理解：

1. lambda 是更方便的写法。
2. 函数对象是更显式、更传统的写法。
3. 它们都可以携带状态，也都可以表现出类似闭包的效果。

如果只是想写一小段临时逻辑，lambda 往往更顺手；如果希望这个可调用对象有明确类型、更多行为，或者需要复用，函数对象通常更合适。
