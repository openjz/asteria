...
令 $\omega_s$ 表示采样频率，$T$ 表示采样周期（二者满足 $\omega_s = \frac{2\pi}{T}$），设 $x(t)$ 是一个带限信号，当 $|\omega| > \omega_m$ 时 $X(j\omega) = 0$ ，如果 $\omega_s > 2\omega_m$，那么 $x(t)$ 可以由其采样值 $x(nT)$ 唯一确定。

已知这些样本值，我们能用如下办法重建 $x(t)$：产生一个周期冲激串，其中冲激幅度就是这些依次而来的样本值；然后将该冲激串通过一个增益为 $T$，截止频率大于 $\omega_M$ 而小于 $\omega_s-\omega_M$ 的理想低通滤波器，该滤波器的输出就是 $x(t)$。

![/img/signals-and-systems/sampling.png](/img/signals-and-systems/sampling.png)

采样后，我们得到的是一个离散序列，而通过低通滤波器后，能将离散信号重建为连续信号。

采样频率 $\omega_s$ 称为**奈奎斯特采样率**。

还有两种不经过低通滤波器的信号重建方法是**零阶保持**和**线性插值**。

零阶保持是让每个采样点的值保持一个采样周期，把一系列脉冲变成了连续的阶梯。

线性插值是直接把相邻两个采样点用直线连起来，形成连续的折线。

### 欠采样

如果采样频率不够高，不满足 $\omega_s > 2\omega_m$，就会发生**混叠**（Aliasing），采样正常的时候 $X(j\omega)$ 的频谱会在 $X_p(j\omega)$ 中重复，但欠采样的时候 $X(j\omega)$ 的频谱会互相重叠，使信号失真。

![/img/signals-and-systems/aliasing.png](/img/signals-and-systems/aliasing.png)

### 离散时间采样

对离散时间信号同样能进行采样，每隔 N 个点做一次采样，得到一个新的离散时间信号。

新信号有很多值为 0 的点，如果我们将值为 0 的点去掉，只保留非 0 点，这个过程叫做**抽取**（Decimation）或减采样，抽取是对采样信号做了一个时间上的压缩。

反过来，如果我们在每个采样点之间插入 N-1 个值为 0 的点，这个过程叫做**内插**（Interpolation）或增采样。
...
拉普拉斯变换是连续时间傅里叶变换在复数域上的推广，傅里叶变化可以看作是拉普拉斯变换的一种特殊情况，当 $s = j\omega$ 时，也就是 $s$ 为虚数时，拉普拉斯变换就退化成了傅里叶变换。
...
z变换是离散时间傅里叶变换在复数域上的推广，离散时间傅里叶变换可以看作是 z 变换的一种特殊情况，当 $z = e^{j\omega}$ 时，也就是 $z$ 在单位圆上时，z 变换就退化成了离散时间傅里叶变换。